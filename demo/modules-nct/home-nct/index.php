...
if(isset($_POST['action']) && ($_POST['action'] == 'getNearByProvidersMech')) {
    $response = array();
    $response = $objHome->getNearByProviders($_POST['latitude'] , $_POST['longitude'] , 'mechanic' , $_POST['vehicle_type']);
    $content = $response['retData']['html'];
    echo json_encode(array('type'=>$response['status']==true ? 'success' : 'error','content'=>$content ));
    exit;
}

if(isset($_POST['action']) && ($_POST['action'] == 'getNearByProvidersTaxi')) {
    $response = array();
    $response = $objHome->getNearByProviders($_POST['latitude'] , $_POST['longitude'] , 'taxi');
    $content = $response['retData']['html'];
    echo json_encode(array('type'=>$response['status']==true ? 'success' : 'error','content'=>$content ));
    exit;
}

if(isset($_POST['action']) && ($_POST['action'] == 'getUserMizutechDetails')){
	$response = $db->pdoQuery("SELECT mizutech_name , mizutech_pwd FROM tbl_users WHERE id = " . $sessUserId)->result();

	$destinationUserName = getTableValue("tbl_users" , "mizutech_name" , array("id" => $_POST['destiUserId']));

	$response['destinationUserName'] = $destinationUserName;
	echo json_encode($response);
	exit;	
}
if(isset($_POST['action']) && ($_POST['action'] == 'getPeername')){
	$response = $db->pdoQuery("SELECT firstName FROM tbl_users WHERE mizutech_name = '".$peername."' ")->result();

	$response['peername'] = ucwords($response['firstName']);
	echo json_encode($response);
	exit;	
}

if( ($sessUserId > 0) && ($_SESSION["user_type"] == 'provider') ) {
