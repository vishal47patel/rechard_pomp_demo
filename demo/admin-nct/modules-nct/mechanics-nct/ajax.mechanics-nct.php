...
if ($action == "updateStatus") {
    if( in_array('status',$Permission)) {
        ...
        echo json_encode(array('type' => 'success', 'Service provider ' . ($value == 'a' ? 'activated ' : 'deactivated ') . 'successfully'));
    ...
    if(in_array('delete',$Permission)){
        ...
        if ($affected_rows && $affected_rows > 0) {
            echo json_encode(array('type' => 'success', 'message' => "Service provider has been deleted successfully."));
            exit;
        } else {
