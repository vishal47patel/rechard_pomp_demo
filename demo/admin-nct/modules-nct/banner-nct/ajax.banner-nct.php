...
if ($action == "updateStatus") {
    if( in_array('status',$Permission)) {
        ...
        echo json_encode(array('type' => 'success', 'Banner ' . ($value == 'a' ? 'activated ' : 'deactivated ') . 'successfully'));
    ...
    if(in_array('delete',$Permission)){
        ...
        if ($affected_rows && $affected_rows > 0) {
            echo json_encode(array('type' => 'success', 'message' => "Banner has been deleted successfully."));
            exit;
        } else {
            echo json_encode(array('type' => 'error', 'message' => "There seems to be an issue deleting Banner."));
            exit;
        }
    }else{
...
$mainObject = new Banner($module, $id, NULL, $searchArray, $action);
