...
webphone_api.onCdr(function (called, connecttime, duration, direction, peerdisplayname, reason, line, callid)
{
...
{
    try{
    webphone_api.common.PutToDebugLog(4, "EVENT, _dialpad: onCreate");
...
{
    try{
    webphone_api.common.PutToDebugLog(4, "EVENT, _dialpad: onStart");
    ...
    if (!webphone_api.common.isNull(document.getElementById('status_dialpad')) && webphone_api.global.dploadingdisplayed === false)
    {
        ...
        webphone_api.common.PutToDebugLogSpecial(1, 'EVENT, _dialpad: onStart display Loading...', false, '');
    }
...
//BRANDEND
    ...
    if (!webphone_api.common.isNull(curruser) && curruser.length > 0) { webphone_api.$('#curr_user_dialpad').html(curruser); }
// set status width so it's uses all space to curr_user
    var statwidth = webphone_api.common.GetDeviceWidth() - webphone_api.$('#curr_user_dialpad').width() - 25;
...
//--    HandleAutoaction();
    ...
    } catch(err) { webphone_api.common.PutToDebugLogException(2, "_dialpad: onStart", err); }
}
...
//also read contacts in background
        setTimeout(function ()
        {
            webphone_api.common.GetContacts(function (success)
            {
                if (!success)
                {
                    webphone_api.common.PutToDebugLog(2, 'EVENT, _dialpad: LoadContacts failed GetCallhistory');
                }
            });
        }, 1000);
    }
...
//--    if (webphone_api.common.HideSettings('recents', '', 'recents', true) === true) { return; }
            ...
            webphone_api.common.HideSettings('recents', '', 'recents', true) === true)
    {
        ...
        MeasureDialPad();
        webphone_api.common.PutToDebugLog(2, "EVENT, _dialpad: PopulateListRecents no recents");
...
//--         intructions Moved after populating is done because MeasuerDialpad() checks the content of the list
    ...
    if (webphone_api.global.recentlist.length < itemstodisplay)
    {
...
//--                webphone_api.global.recentlist[i][webphone_api.common.RC_NAME] = webphone_api.global.recentlist[i][webphone_api.common.RC_NUMBER];
    ...
    webphone_api.common.PutToDebugLog(2, 'EVENT, _dialpad Starting populate recents list');
...
// if list height greater than available space, the hide dialpad
    ...
    } catch(err) { webphone_api.common.PutToDebugLogException(2, "_dialpad: PopulateListRecents", err); }
}
...
{
    try{
    if (webphone_api.common.isNull(searchval) || (webphone_api.common.Trim(searchval)).lengh < 1)
    {
...
// --------------------------
    ...
    if (webphone_api.common.GetConfigBool('hasfiletransfer', true) !== false && (webphone_api.common.GetConfigBool('usingmizuserver', false) === true || webphone_api.common.IsMizuWebRTCGateway() === true))
    {
//OPSSTART
...
//--    if (webphone_api.$('#footertext_dialpad').is(':visible')) { contentHeight = contentHeight - webphone_api.$("#footertext_dialpad").height(); }
    ...
    var brandW = webphone_api.$("#dialpad_additional_header_left").width() - 5;
...
//--            && webphone_api.global.useengine !== webphone_api.global.ENGINE_WEBPHONE)
//--            {
//--                ;
//--            }else
//--            {
...
// remove data transition for windows softphone, because it's slow
    ...
    if ( webphone_api.common.isNull(menuId) || menuId.lenght < 1 ) { webphone_api.common.PutToDebugLog(2, "ERROR, _dialpad: CreateOptionsMenu menuid null"); return; }
...
// handle hidesettings
    ...
    {
        if ((webphone_api.common.GetConfigBool('usingmizuserver', false) === true || webphone_api.common.IsMizuWebRTCEmbeddedServer() === true || webphone_api.common.IsMizuWebRTCGateway() === true) &&
...
//OPSEND
    ...
    if ((webphone_api.common.getuseengine() === webphone_api.global.ENGINE_WEBRTC && (webphone_api.common.GetBrowser() === 'Firefox' || webphone_api.common.GetBrowser() === 'Chrome'))
            || webphone_api.global.audio_devices_loaded === true && (webphone_api.common.GetParameter('devicetype') === webphone_api.common.DEVICE_WIN_SOFTPHONE() || webphone_api.common.getuseengine() === webphone_api.global.ENGINE_SERVICE || webphone_api.common.getuseengine() === webphone_api.global.ENGINE_WEBPHONE))
    {
//BRANDSTART
        if (webphone_api.common.GetConfigInt('brandid', -1) !== 50) // favafone
//BRANDEND
...
{
    try{
    webphone_api.common.PutToDebugLog(4, "EVENT, _dialpad: onStop");
