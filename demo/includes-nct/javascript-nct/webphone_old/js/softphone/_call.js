...
{
    try{
    webphone_api.common.PutToDebugLog(4, "EVENT, _call: onCreate");
    ...
    {
        webphone_api.common.PutToDebugLog(4, 'EVENT, _call Hangup onclick');
        HangupCall();
    });
    
    webphone_api.$("#btn_accept").on("click", function()
    {
        webphone_api.common.PutToDebugLog(4, 'EVENT, _call AcceptCall onclick');
        AcceptCall(true);
    });
...
{
    try{
    webphone_api.common.PutToDebugLog(4, "EVENT, _call: onStart");
...
//--        }
    ...
    if (calltype === "incoming")
    {
        if (webphone_api.common.GetParameterBool('autoaccept', false) === true)
        {
            webphone_api.$('#hangup_layout').show();
            webphone_api.$('#callfunctions_layout').show();
            webphone_api.$('#acceptreject_layout').hide();
...
function NormalizeDisplayDetails(det_in)
{
    ...
    {
        if (webphone_api.common.isNull(darr[idx]) || webphone_api.common.Trim(darr[idx]).lengh < 1)
        {
            darr.splice(idx, 1);
        }else
        {
    ...
    {
        if (webphone_api.common.isNull(darr[i]) || webphone_api.common.Trim(darr[i]).lengh < 1) { continue; }
...
{
    try{
    var ep = webphone_api.common.GetEndpoint(webphone_api.global.aline, '', '', false);
    if (webphone_api.common.isNull(ep) || ep.length < 5)
    {
        webphone_api.common.PutToDebugLog(2, 'ERROR, _call OnNewIncomingCall: ep is NULL for line: ' + webphone_api.global.aline.toString());
        return;
    }
...
*/
        webphone_api.plhandler.Reject(linetoreject);
...
function AcceptHold(callapi)
{
    ...
    if (callapi)
    {
        ...
        if (prevline > 0)
        {
            ...
            webphone_api.plhandler.Hold(true, prevline);
            setTimeout(function ()
            {
                AddCallFunctions(false);
            }, 250);
...
function AcceptEnd(callapi)
{
    ...
    if (callapi)
    {
        ...
        if (prevline > 0)
        {
            ...
            webphone_api.plhandler.Hangup(prevline);
        }
    }
...
//--    webphone_api.$('#speaker_status').removeClass("callfunc_status_on");
    ...
    webphone_api.common.PutToDebugLog(4, 'EVENT, _call HangupCall');
...
{
    try{
    webphone_api.common.PutToDebugLog(3, 'EVENT, _call CloseCall');
    webphone_api.$.mobile.back();
...
//--{
        if (webphone_api.global.aline == iline)
        {
            if (webphone_api.common.GetParameter('automute') == '4')
            {
...
// call directly to engine API files to be able to pass exact line
                    ...
                    else if (webphone_api.common.getuseengine() === webphone_api.global.ENGINE_WEBPHONE)
                    {
            ...
            if (webphone_api.common.GetParameter('autohold') == '4')
            {
                if (item[webphone_api.common.EP_HOLDSTATE] == 'true') // if is on hold
                {
                    ...
                    if (webphone_api.common.getuseengine() === webphone_api.global.ENGINE_WEBRTC)
                    {
                        webphone_api.webrtcapi.SipToggleHoldResume(false, iline);
                    }
                    else if (webphone_api.common.GetParameter('devicetype') === webphone_api.common.DEVICE_WIN_SOFTPHONE() || webphone_api.common.getuseengine() === webphone_api.global.ENGINE_SERVICE)
                    {
                    ...
                    webphone_api.global.ep[i][webphone_api.common.EP_HOLDSTATE] = 'false';
                }
            }
        }else
        {
            ...
            if (webphone_api.common.GetParameter('autohold') == '4')
            {
                if (item[webphone_api.common.EP_HOLDSTATE] == 'false') // if not on hold
                {
                    ...
                    if (webphone_api.common.getuseengine() === webphone_api.global.ENGINE_WEBRTC)
                    {
                        webphone_api.webrtcapi.SipToggleHoldResume(true, iline);
                    }
                    else if (webphone_api.common.GetParameter('devicetype') === webphone_api.common.DEVICE_WIN_SOFTPHONE() || webphone_api.common.getuseengine() === webphone_api.global.ENGINE_SERVICE)
                    {
                    ...
                    webphone_api.global.ep[i][webphone_api.common.EP_HOLDSTATE] = 'true';
                }
            }
        }
//--}
...
function Conference(phoneNr) // popup
{
    ...
    if (webphone_api.common.getuseengine() === webphone_api.global.ENGINE_WEBRTC
            && webphone_api.common.GetConfigBool('usingmizuserver', false) === false && webphone_api.common.IsMizuWebRTCEmbeddedServer() === false && webphone_api.common.IsMizuWebRTCGateway() === false)
    {
...
// if is multiline, then try to find another active call and connect them whitout asking for number
    if (webphone_api.common.GetNrOfActiveCalls() > 1)
    {
        for (var i = 0; i < webphone_api.global.ep.length; i++)
        {
            ...
            if (ln !== webphone_api.global.aline) // we found another active call
            {
                ...
                {
                    if(webphone_api.common.IsMizuServer() === true && IsConferenceRoom() === true) // means it's conference rooms, so just send invites via chat
                    {
    ...
    if (webphone_api.common.GetNrOfActiveCalls() > 1 && (webphone_api.common.getuseengine() === webphone_api.global.ENGINE_SERVICE || webphone_api.common.getuseengine() === webphone_api.global.ENGINE_WEBPHONE || webphone_api.common.IsWindowsSoftphone() === true))
    {
        interconnectHtml = '<br />'+
...
//-- remove data transition for windows softphone, because it's slow
    ...
    if ( webphone_api.common.isNull(menuId) || menuId.lenght < 1 ) { webphone_api.common.PutToDebugLog(2, "ERROR, _call: CreateOptionsMenu menuid null"); return; }
    ...
    if (featureset > 5)
    {
        if (availableFunc.indexOf('conference') >= 0 &&
            (
                ...
                || (webphone_api.common.getuseengine() === webphone_api.global.ENGINE_WEBRTC && (webphone_api.common.GetConfigBool('usingmizuserver', false) === true || webphone_api.common.IsMizuWebRTCEmbeddedServer() === true || webphone_api.common.IsMizuWebRTCGateway() === true || webphone_api.common.CanIUseService() === true))
            ))
        
//--        if (availableFunc.indexOf('conference') >= 0 || (webphone_api.common.IsMizuServer() === true && IsConferenceRoom() === true)) // for conference rooms
        {
...
//--    }
    ...
    if ((webphone_api.common.getuseengine() === webphone_api.global.ENGINE_WEBRTC && (webphone_api.common.GetBrowser() === 'Firefox' || webphone_api.common.GetBrowser() === 'Chrome'))
        || webphone_api.common.GetParameter('devicetype') === webphone_api.common.DEVICE_WIN_SOFTPHONE() || webphone_api.common.getuseengine() === webphone_api.global.ENGINE_SERVICE || webphone_api.common.getuseengine() === webphone_api.global.ENGINE_WEBPHONE)
    {
...
{
    try{
    webphone_api.common.PutToDebugLog(4, "EVENT, _call: onStop");
