...
class PdoWrapper extends PDO {
    ...
    public function pdoQuery( $sSql = '', $aBindWhereParam = array() ) {
        ...
        if ( !in_array( $operation[0], $this->aValidOperation ) ) {
            self::error( 'invalid operation '.$operation[0].' called in query. use only ' . implode( ', ', $this->aValidOperation ) );
        }
            ...
            if(strstr($f, ' ')){
                ...
                switch ( gettype( $array[$f] ) ):
                    ...
                    case 'boolean':
                        ...
                        break;
                    // if float found then set pdo param as string
                    case 'float':
                        $this->_oSTH->bindParam( ":s" . "_" . "$field", $array[$f], PDO::PARAM_STR );
                        break;
                    // if double found then set pdo param as string
                    case 'double':
                        $this->_oSTH->bindParam( ":s" . "_" . "$field", $array[$f], PDO::PARAM_STR );
                        break;
                ...
                switch ( gettype( $array[$f] ) ):
                    ...
                    // if null found then set pdo param as null
                    case 'NULL':
                        $this->_oSTH->bindValue(":s" . "_" . "$f", $array[$f], PDO::PARAM_NULL);
                        break;
                endswitch;
            } // end for each here
        }
    ...
    private function _bindPdoParam( $array = array() ) {
        ...
        foreach ( $array as $f => $v ) {
            ...
            switch ( gettype( $array[$f] ) ):
                ...
                // if null found then set pdo param as null
                case 'NULL':
                    $this->_oSTH->bindValue( $f + 1, $array[$f], PDO::PARAM_NULL);
                    break;
            endswitch;
        } // end for each here
    }
    ...
    public function error( $msg ) {
        ...
        /*@sendEmailAddress('gaurav.chavda@ncrypted.com', 'Error in '.SITE_NM, SITE_NM.' '.date( 'Y-m-d h:m:s' ) . ' :: ' . $msg . "\n".'URL: '.$_SERVER["SERVER_NAME"] . $_SERVER["REQUEST_URI"]);*/
    }
    /**
